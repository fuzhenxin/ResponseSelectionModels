...
def dense(x, out_dimension=None, add_bias=True, init=None):
...
def mask(row_lengths, col_lengths, max_row_length, max_col_length):
    ...
    if len(row_mask.shape)==4:
        row_mask = tf.reshape(row_mask, [-1, row_mask.shape[1]*row_mask.shape[2], 1])
        col_mask = tf.reshape(col_mask, [-1, col_mask.shape[1]*col_mask.shape[2], 1])
