...
def masked_softmax(scores, mask):
    ...
    """
    numerator = tf.exp(tf.subtract(scores, tf.reduce_max(scores, 2, keepdims=True))) * tf.expand_dims(mask, axis=1)
...
def normalize(inputs, axis=None, epsilon=1e-8, scope='normalize', reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # mean, variance = tf.nn.moments(inputs, [-1], keepdims=True) # more fast
